...
def _get_server_venv_site(
    ...
    try:
        site_packages: list[str] = json.loads(proc.stdout)
    except ValueError:
        return None, []
...
def _get_server_version(
    ...
    try:
        return Version.parse(version_line), devmode_python
    except ValueError:
        raise InstanceError(
            f"could not check version of server located at {executable}: "
...
class PersistentInstance(ManagedInstance):
    def __init__(
        ...
        *,
        ...
        self._server_cmd.append(f"--data-dir={self._data_dir}")

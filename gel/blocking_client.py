...
class BlockingIOConnection(base_client.BaseConnection[threading.Event]):
    ...
    async def close(self, timeout: float | None = None) -> None:
        ...
        print(ident, 'BlockingIOConnection.close() called')
        if not self.is_closed():
            try:
                ident += f" {id(self._protocol)}"
                print(ident, 'BlockingIOConnection try to terminate protocol')
                self._protocol.terminate()
                print(ident, 'BlockingIOConnection wait for protocol to disconnect, timeout', timeout)
                if timeout is None:
                ...
                print(ident, 'BlockingIOConnection protocol disconnected')
            except TimeoutError:
                print(ident, 'BlockingIOConnection protocol timeout')
                self.terminate()
                raise errors.QueryTimeoutError()
            except Exception:
                print(ident, 'BlockingIOConnection exception during close')
                self.terminate()
                raise
            finally:
                self._cleanup()
...
class _PoolConnectionHolder(
    ...
    async def close(
        ...
        if self._con is None:
            print(ident, "holder.close() called, but no connection")
            return
        print(ident, "holder.close() called", timeout)
    ...
    async def wait_until_released(self, timeout: float | None = None) -> None:
        ...
        print(ident, "holder.wait_until_released() called", timeout)
        result = self._release_event.wait(timeout)
        if timeout is not None and not result:
            print(ident, "holder.wait_until_released() timeout")
            raise TimeoutError
        print(ident, "holder.wait_until_released() done")
...
class _PoolImpl(
    ...
    def __init__(
        ...
        *,
        ...
        ident = f"{threading.get_ident()} {id(self)}"
        print(ident, "_PoolImpl.__init__()")
        import traceback
    ...
    async def close(self, timeout: float | None = None) -> None:
        ...
        self._closing = True
        ident = f"{threading.get_ident()} {id(self)}"
        print(ident, 'PoolImpl.close() called', timeout, len(self._holders))
        import traceback
        traceback.print_stack(file=sys.stdout)
        try:
            if timeout is None:
            ...
            print(ident, 'PoolImpl.close() timeout', timeout)
            self.terminate()
            raise errors.InterfaceError(
            ...
            print(ident, 'PoolImpl.close() exception', e)
            self.terminate()
            raise
        finally:
            self._closed = True
            self._closing = False
        print(ident, 'PoolImpl.close() done')
